...
- **The root `bunfig.toml` is load-bearing.** `bun test` matches `*.spec.ts` as well as `*.test.ts`, so without `[test] pathIgnorePatterns` the root test job collects the site's Playwright specs and dies on the first `test.describe()`. If root test discovery ever changes from 30 files, check that file first.
