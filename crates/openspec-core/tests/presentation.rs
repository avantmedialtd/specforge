...
use openspec_core::{PaletteColor, PresentationKey, WorkspacePresentationStore, WorkspaceRegistry};
use std::fs;
...
fn register_then_set_presentation_survives_restart() {
    ...
    assert!(
        reg.entry(&ws).is_some(),
        "workspace registration should restore"
    );
...
fn repo_presentation_survives_until_last_user_registered_workspace_is_gone() {
    ...
    store
        .set(key.clone(), Some("MyRepo".into()), Some(PaletteColor::Rose))
        .unwrap();
    ...
    assert!(
        ...
        "Repo presentation must survive partial unregister"
    );
