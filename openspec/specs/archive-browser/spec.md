...
Defines the Archive view: a read-only browser over one top-level row's archived changes — a repository group or a flat workspace — rendered in the detail pane and reached from the sidebar footer's Archive entrypoint. For a repository it pools `openspec/changes/archive/` across **every tracked worktree**, including auto-discovered ones, and de-duplicates on the bare logical change id, so a change archived inside a feature worktree is reachable before its branch merges. It covers the scope selector, the newest-first listing whose identifier and `YYYY-MM-DD` date come from the `<YYYY-MM-DD>-<id>` directory name, the in-memory search filter, per-change artifact navigation backed by the `list_archived_rows` and `archived_artifact_status` commands, the per-change control for choosing which worktree's copy is read, live refresh on archive-transition events only while the view is open, and the confinement of every archive read to registered or registry-discovered workspaces. Its defining constraint is that archived content is loaded on demand and never parsed on the watcher's aggregation path — the active-change tree, the footer entrypoint itself, and the shared markdown rendering this view reuses belong to `spec-browser`.
