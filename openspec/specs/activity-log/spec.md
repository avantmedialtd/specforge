...
Defines the append-only achievement log behind SpecForge's progress views: task completions, artifact statuses, change creations, and archivals observed live when the watcher re-parses a registered workspace, plus a bounded backfill reconstructed from git history, each event stamped with the raw `(name, email)` author it was seen with rather than a pre-resolved developer. The log persists in the application's data directory as `activity.json` — never inside a workspace's `openspec/` tree, and never via a git operation that mutates history or the working tree — and answers bounded queries bucketed by the viewer's local calendar day. It owns recording, storage, and querying only: the Dashboard surfaces that read it (today, streak, heatmap, leaderboard) are specified by `dashboard`, and the rules that fold a raw author onto a canonical developer by `developer-identity`.
