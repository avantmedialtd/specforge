# bitbucket-pull-requests Specification

## Purpose

Defines the opt-in BitBucket pull-request panel for SpecForge: a background poller in the headless app layer that lists the pull requests the configured account authored across every workspace it belongs to (the `/user`, `/user/workspaces`, `/workspaces/{ws}/pullrequests/{account}` recipe, read-only, with review state folded into the list response), credentials stored in the application settings write-only and overridable from the environment, polling with caching and backoff, a payload-less cache event announcing each new snapshot, a collapsible panel rendered in one of four persisted side-pane slots in the desktop application and the browser skin, how a pull request is opened on each transport, the privacy posture, and the terminal frontend's deliberate toggle-only stance.
## Requirements
