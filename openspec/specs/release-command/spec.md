...
On approval the command SHALL commit the notes file, create the target tag on that commit by invoking the existing version-bump script, and push `master` together with the tag.
