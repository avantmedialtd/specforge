# reader-window Specification

## Purpose

Defines the **reader window**: a detached, chromeless surface showing exactly one markdown document and nothing that navigates. Covers what it contains and omits, the two ways it is launched, how a second launch for the same document is deduplicated to a focus, its title and titlebar treatment, its shared geometry, its dismissal paths, how it stays fresh, what it does when its document vanishes, and why the reader presentation is deliberately not part of the Address.
