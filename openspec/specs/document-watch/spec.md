# document-watch Specification

## Purpose

Defines the **per-document filesystem subscription** shared by every surface that renders one markdown document. Covers registration and release keyed by browse root and relative path, ownership by the frontend holding it, reference counting across surfaces, parent-directory watching for atomic-rename durability, the `document-changed` event and payload, the bound on how many watches may exist, and why this operates independently of the workspace watcher that maintains the change cache.
