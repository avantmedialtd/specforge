...
Defines how SpecForge measures the *quality* of its test suite rather than its
extent. Tests can execute a line without asserting anything about it, and
coverage cannot tell the difference; mutation testing injects small behavioural
bugs and reports which ones no test catches. This capability covers the scope
that is mutated and why the frontend shells are excluded, the requirement that
the baseline stay green (and why suppressing that check is forbidden), the
continuous-integration gate that fails a push whose *own changed lines* carry a
surviving mutant, the timeout policy, and the local commands plus the recorded
full-sweep baseline. Surviving mutants elsewhere in the codebase are a backlog,
never a build break.
