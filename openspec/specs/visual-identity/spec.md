...
The content column SHALL be two-tier. The column itself — the width available to object blocks (tables, fenced code, diagrams, SVG images, display mathematics) — SHALL be bounded by the configured reading width, whose default is 880px, at which fenced code at `--text-md` renders roughly 94 characters per line. Prose blocks — paragraphs, lists, and blockquotes — SHALL additionally be limited to a readable measure of their own font, defaulting to a measure between 70ch and 80ch, so body text wraps well short of the roughly 114 characters the default column would otherwise produce. Headings SHALL keep the full column, so the hairline rules beneath `h1` and `h2` span the whole reading surface; object blocks SHALL likewise keep the full column so wide content does not pay for the prose measure.

Three figures in the paragraph above were re-measured for this revision and differ from the numbers it previously carried: fenced code at the 880px column is ~94 characters rather than ~97 (the earlier figure did not subtract the code well's own padding), the unmeasured column runs ~114 rather than ~110, and the prose measure is as described next. Only the last changes any claim this requirement makes; the other two are recorded so the correction is traceable rather than silent.

The `ch` figures above SHALL NOT be read as character counts. `ch` is the advance of the digit zero, which in the prose font is about 1.3× the width of an average prose character, so the default 74ch measure renders approximately **97** characters per line — not the 74 the unit's name suggests, and not the ~78 an earlier statement of this requirement claimed. The default measure is therefore *above* the 60–90 characters conventionally called comfortable for continuous reading. That is recorded here as a known property of the default rendering rather than corrected, because it is what every existing installation already renders; a reader who wants a comfortable measure selects a narrower reading width (see the `document-width` capability). Any future retune of the default belongs to a change that argues for it on typographic grounds.
...
- **THEN** the paragraph's text wraps at a measure between 70ch and 80ch — approximately 97 rendered characters
