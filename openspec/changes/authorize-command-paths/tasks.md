...
- [ ] 1.1 Add `ensure_registered_repo(&self, repo_id: &Path) -> Result<RepoId, String>`
...
- [ ] 1.2 Add `ensure_registered_workspace(&self, workspace: &Path) -> Result<PathBuf, String>`:
  canonicalize, compare against the canonicalized `registry.entries()` folder set,
  error on miss.
- [ ] 1.3 Call `ensure_registered_repo` at the top of `commit_graph`,
  `commit_detail`, `commit_diff`.
- [ ] 1.4 Call `ensure_registered_workspace` at the top of `read_artifact`,
  `list_archived`, `archived_artifact_status`.

## 2. Converge the divergent desktop commands onto AppService
- [ ] 2.1 `commands.rs`: give `get_commit_graph` / `get_commit_detail` /
...
- [ ] 2.2 `commands.rs`: give `list_archived` and `archived_artifact_status` an
...
- [ ] 2.3 Confirm `dispatch.rs` needs no change (already routes through
  `AppService`), so both transports now share the guard.

## 3. Tests (openspec-app)
- [ ] 3.1 Unregistered `repo_id` is refused by `commit_graph` / `commit_detail` /
  `commit_diff`; a registered repo still returns results.
- [ ] 3.2 Unregistered `workspace` is refused by `read_artifact` / `list_archived`
  / `archived_artifact_status`; a registered workspace still succeeds.
- [ ] 3.3 Canonicalization: a registered path spelled with a trailing slash / `..`
  / symlink resolves to the same accept decision (membership isn't spelling-
  sensitive).

## 4. Verify
- [ ] 4.1 `cargo test -p openspec-app` and `cargo test -p openspec-core` green.
- [ ] 4.2 `cargo build` (workspace) clean; `cargo test -p specforge` (or the shell
  crate) builds with the new `svc` params.
- [ ] 4.3 Manual/desktop smoke: selecting a commit and opening the Archive view in
  a registered workspace still works unchanged.
- [ ] 4.4 `openspec validate authorize-command-paths --strict` passes.
