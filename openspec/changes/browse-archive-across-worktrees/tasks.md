...
- [ ] 7.15 **Archive-time only.** `openspec/specs/archive-browser/spec.md`'s `## Purpose` block still calls the view a "single-workspace browser" and names the now-removed `list_archived` command. A delta can only carry requirements, so this prose must be updated when the change is archived and its specs synced.
