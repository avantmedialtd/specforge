...
The wording is deliberately narrow. A broader phrasing — "no per-author comparison of any kind" — would contradict the commit garden, which *is* a per-author surface on the Dashboard, and would collide with *Per-Repository Breakdown*, which orders **repositories** by active-change count descending. (*Dashboard Includes Disabled Workspaces* is the wrong citation for that ordering — it mandates none, and only defers to *Per-Repository Breakdown* for the ranking keys.) Ranking authors against one another is the thing being forbidden; showing several authors is not.
...
- **`IdentityConfig::label()` loses both production call sites** (`dashboard.rs:328` and `garden.rs:101`) and becomes a `pub` method in a library crate whose only exerciser is its own unit test — so `dead_code` never fires and CI stays green. Its rule (display_name → primary alias display → "You") is also reimplemented in TypeScript with divergent fallbacks (`DashboardView.tsx`'s `identityNameOf` returns `null` where the core returns "You"). *Mitigation:* keep `label_prefers_display_name_then_primary_then_you` regardless — it is the method's only remaining coverage. Do **not** justify keeping it as the last exerciser of `Author::display()`: `garden.rs`'s `RawCommit` mapping still calls `display()`, and the new `other_authors_key_on_their_raw_identity` asserts through it.
