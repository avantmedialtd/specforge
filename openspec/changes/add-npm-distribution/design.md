...
- **Trusted publishing cannot perform a package's first publish, so the automated channel cannot bootstrap itself.** npm stores a trusted-publisher configuration *on a package*, and that configuration can only be created once the package exists. There is no scope-level publisher, no name reservation, and no pending-publisher mechanism; the npm CLI issue requesting first-publish-over-OIDC is still open. Left unaddressed, the first tag fails on the very first `npm publish` — after the GitHub Release is already public. → Each of the six names gets a one-time manual placeholder publish, done by hand with interactive 2FA, after which its trusted publisher is configured and every subsequent publish is automated and attested. This is a permanent property of the channel, not a one-off: **adding a sixth platform later requires the same manual bootstrap for that new package name.** The runbook lives in `npm/README.md`.
- **Trusted publishing also has a floor on the npm CLI itself.** It requires npm ≥ 11.5.1, and the Node 22 runner image bundles npm 10, which never attempts the OIDC exchange — a failure that would look like bad credentials rather than an old client, again after the release is public. → The publish job pins Node 24 and installs an explicit npm range, printing the version so the log proves OIDC was possible.
