...
`pre` and `.katex-display` get authored `margin: 0.8em 0`, joining tables/diagrams/quotes on the object tier; paragraphs/lists stay on the prose tier. `section[data-footnotes]` gets a top rule, `margin-top: 1.5em`-class spacing, `--text-sm` at `--text-muted`; a real `.sr-only` utility rule is added so the GFM "Footnotes" heading is hidden deliberately rather than by a third-party stylesheet accident. Prose measure per the spec: `p, ul, ol, blockquote { max-width: 74ch }` inside the 880px column; headings and object blocks span the column.
