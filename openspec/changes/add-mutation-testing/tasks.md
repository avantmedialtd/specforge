...
- [ ] 4.1 Run the full sweep (`cargo mutants --no-shuffle -j4`) and record its date, scope, counts and wall-clock in a `README.md` table sourced from `mutants.out/outcomes.json` (`mutation-testing`: *Local invocation and recorded baseline*)
- [ ] 4.2 Add command-table rows to `README.md` — reproduce the gate locally, list mutants without building, mutate a single file — plus a row in the CI table noting `mutants.yml` is a separate workflow (`mutation-testing`: *Local invocation and recorded baseline*)
- [ ] 4.3 Add the local command to `CLAUDE.md`'s command table and a Conventions paragraph covering the gate, the written-reason exclusion rule, and the prohibition on suppressing the baseline check (`mutation-testing`: *Green baseline prerequisite*, *Local invocation and recorded baseline*)
...
- [ ] 5.3 Push the branch and confirm the **Mutants** workflow behaves on both paths: a commit touching no in-scope Rust short-circuits in under a minute, and a commit touching in-scope Rust runs mutants and annotates
- [ ] 5.4 Confirm the gate actually gates — introduce a deliberately unasserted change in an in-scope crate on a scratch commit, confirm the job goes red, then drop it
