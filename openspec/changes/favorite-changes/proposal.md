...
- **Persistence is a settings id-list mutated by delta.** A new favorites id-list lives in `AppSettings` (config-dir `settings.json`), hydrated at tree mount; toggles are recorded as pending ops and flushed on a debounce as an add/remove **delta** that the backend merges under its settings lock — so a failed hydration can never echo-write an empty list over stored favorites and concurrent clients compose instead of clobbering. A get command and the delta-update command are exposed by the desktop shell and mirrored in the web dispatch table, per the web UI's command-mirror contract. Ordering is applied entirely in the frontend; core continues to emit alphabetical views.

```mermaid
flowchart LR
    click[Star click / key toggle] --> set[favorites Set + pending ops]
    set --> render[Re-render: starred partition first,\nalphabetical within each]
    set -- debounced delta --> cmd[update_favorite_change_ids]
    cmd -- merge under lock --> json[(settings.json)]
    cmd -- merged list --> set
