...
- **No motion, no glow, no full-row wash, no new token.** The completion disc carries no `--glow-ok` halo (glows stay reserved for the in-progress meter and the other sanctioned surfaces); completion never washes the row background (a full-row wash still means *selected* and nothing else); and `--ok` already exists in both schemes, so no design token is added.
...
- `src/App.css` — style the completion disc (≈14–16px, `--ok` fill, `--surface` check, no glow, visually distinct from the 4px status dots); add the `tree-row--complete` `--ok` rail rule below the existing `.selected` rail override; change `.tree-row--struck .row-label` colour from `--text-faint` to `--ok` while retaining `text-decoration: line-through`.
...
- **Deliberate scope boundaries** (so nobody "fixes" these later as oversights):
  ...
  - **No full-row completion wash.** A completed row is not tinted; the full-row wash remains the exclusive signal of selection. (Dropping the wash is also why no `--ok-tint` token is needed.)
