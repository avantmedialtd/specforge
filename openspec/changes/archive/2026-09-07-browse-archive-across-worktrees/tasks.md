...
Raised by `/code-review max` after the first implementation pass. Six of the
findings had one root cause, so they are grouped under 7.1.

- [x] 7.1 **Root.** `build_repo_view` bucketed a mixed logical change into `active` whole, so content-less archive stubs reached the SERIALIZED section every frontend and the TUI read — and nothing checks `is_archived_here`. Partition instances by flavour instead: compute divergence over the joined set (which is what makes `[stale]` fire), then emit active instances to `active` and archived ones to `archived`, under the same name. Collapses the tree's unlabelled unclickable row, the address resolver demanding an instance segment for a singleton, the TUI reading `openspec/changes/<dated-dir>/`, `summary_metrics` folding a 0/0 stub, the drained `archived_count`, and the duplicate React key when one worktree holds both flavours.
- [x] 7.2 Pin it: `no_archived_instance_ever_reaches_the_serialized_active_section` (every mixed shape at once) and `one_worktree_holding_both_flavours_yields_no_duplicate_instance_key`. Re-point `glyph_predicate.rs`, whose comment asserted the leak was harmless "solely because `list_archived_stubs` synthesises `ArtifactStatus::default()`" — now structural.
- [x] 7.3 `repo_ships` joined `archived_at` on the bare logical id, which yields the EARLIEST archival across every dated directory of that id — stamping today's ship with another day's instant. Add `ChangeLifecycle::archived_at_by_dir` (per archive directory) and join on the instance's own directory. Covered by `a_ship_carries_its_own_directorys_instant_not_the_ids_earliest`.
- [x] 7.4 `missing_lifecycle_events` aliased EVERY persisted `ChangeArchived` id to its date-stripped form, so a change genuinely named `2026-06-05-add-thing` would permanently suppress the archival of a different change called `add-thing`. Draw the alias from evidence instead — only when git's lifecycle set contains the stripped form and NOT the dated one. Covered by `a_date_headed_change_id_does_not_suppress_a_different_change`.
- [x] 7.5 `list_archived_rows` propagated the first per-worktree error with `?`, so one unreadable worktree replaced the whole repository's union with an error banner. Degrade per worktree (`unwrap_or_else` + log), matching the aggregation path's `list_archived_stubs(...).unwrap_or_default()`.
- [x] 7.6 `ArchiveView` scoped its selector to `views`, which `workspace_views` strips of disabled rows — making a parked row's archive unbrowsable even though the readers authorize on registry membership. Append parked rows from `workspaces`, labelled `(parked)`.
- [x] 7.7 A deep link whose worktree matched no scope fell back to `scopes[0]`, silently listing an unrelated repository. Match a repo's `mainWorktree` as well as its `worktrees` (the two are built from different sources and disagree when the main checkout has no `openspec/`).
- [x] 7.8 The artifact fallback picked `proposal` without checking the newly-selected copy has one — leaving no tab rendered and a read error. Fall back to `firstPresentArtifact`.
- [x] 7.9 `pendingOpen` was cleared only on a match, so an unmatched deep link stayed armed and a later refresh yanked the reader away. Clear it on scope change and after the first completed load that does not contain it.
- [x] 7.10 `activeCopy` stayed null on a row click, so the reader tracked whatever `copies[0]` currently was and a background refresh could re-point it at another worktree. Pin the copy at open time.
- [x] 7.11 Reader header showed the ROW's date while the body rendered the selected copy; `copyLabels`' collision disambiguator appended the archive directory, a no-op in exactly the case it detects. Date from the copy; disambiguate a base-label collision by worktree path.
- [x] 7.12 Refetch the union when a scope's worktree SET changes, not only on archive events; coalesce the paired `change-archived` / `logical-change-archived` into one refetch rather than running the N-worktree fan-out twice per archival.
- [x] 7.13 Remove dead code the change orphaned: the `list_archived` command across its (now three) registration points plus the `listArchived` wrapper — nothing calls it, while `AppService::list_archived` stays because the confinement requirement specifies and tests it. Drop `worktreeForHint`'s two legacy passes, provably unreachable because `gather_repo_inputs` builds `RepoView.worktrees` from every registry entry with no filesystem check, making it a superset of both.
- [x] 7.14 Spec deltas for the above: `spec-browser`'s *Workspace Tree Hierarchy* (the grouping key is the logical identifier, and instances are partitioned so no archived one is rendered) and `archive-browser`'s *Search Within the Selected Workspace's Archive* (filters the union; a pooled change matches once).
- [x] 7.15 **Archive-time only.** `openspec/specs/archive-browser/spec.md`'s `## Purpose` block still calls the view a "single-workspace browser" and names the now-removed `list_archived` command. A delta can only carry requirements, so this prose must be updated when the change is archived and its specs synced.
