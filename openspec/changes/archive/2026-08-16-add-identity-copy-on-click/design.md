...
- ~~**The macOS clearance is verified by simulation, not in the native window.**~~ **Resolved.** Verified in the running Tauri window (task 9.1): with the real platform flag and the strip genuinely live, the change name hit-tests to itself, the strip band still hit-tests to the drag region, and WKWebView computes `-webkit-user-select: all` on the span — the one thing simulation could not cover and the reason Decision 5 avoided a real `<button>`. The same probe measured a fact the design had only inferred: the Tauri WebView **is** a secure context and **does** expose `navigator.clipboard.writeText`, so the desktop app takes the async path and the synchronous fallback exists solely for non-loopback plain-HTTP origins.
