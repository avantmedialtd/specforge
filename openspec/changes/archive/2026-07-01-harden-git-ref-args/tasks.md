...
- [x] 1.1 In `crates/openspec-core/src/git.rs`, insert `--end-of-options`
  immediately before `sha` in `commit_diff`
  (`["show", "--format=", "--end-of-options", sha, "--", path]`).
- [x] 1.2 In `diff_tree_lines`, insert `--end-of-options` before `sha`
  (`["diff-tree", "--no-commit-id", "-r", mode, "--end-of-options", sha]`),
  keeping `mode` before the terminator.
- [x] 1.3 Audit the other ref-taking helpers (`commit_log`, `commit_files`,
...
- [x] 2.1 Add a shared predicate (e.g. `is_object_id(&str) -> bool` matching
  `^[0-9a-fA-F]{4,64}$`) in `openspec-core` (git module) exported for reuse.
- [x] 2.2 In `crates/openspec-app/src/service.rs`, reject a non-matching `sha` in
...
## 3. Tests
- [x] 3.1 `openspec-core` test: calling `commit_diff` / `commit_files` with
  `sha = "--output=<tmp>"` leaves `<tmp>` non-existent afterward (no file
  written) and returns empty/inert.
- [x] 3.2 `openspec-core` test: `is_object_id` accepts 7- and 40-char hex,
  rejects `--output=x`, `HEAD`, `:/msg`, empty, and over-length input.
- [x] 3.3 Regression: a real full and abbreviated sha still return the expected
  diff and file list.

## 4. Verify
- [x] 4.1 `cargo test -p openspec-core` and `cargo test -p openspec-app` green.
- [x] 4.2 `cargo build` (workspace) clean; no frontend/type changes needed.
- [x] 4.3 `openspec validate harden-git-ref-args --strict` passes.
