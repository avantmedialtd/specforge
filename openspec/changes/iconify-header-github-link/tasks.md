...
- [ ] 1.1 Obtain GitHub's official `mark-github` path (16×16 `viewBox`) from its
...
- [ ] 1.2 Add a local `GitHubMark` function to `site/src/Layout.tsx`, directly after
...
- [ ] 1.3 Do not create `site/src/components/icons.tsx`, and do not add an icon
  package to `site/package.json` — the design records why both were rejected.

## 2. The navigation link

- [ ] 2.1 In `site/src/Layout.tsx`, replace the `GitHub` text inside the
  ...
  labelling it*).
- [ ] 2.2 Add `aria-label="GitHub"` to that anchor so its accessible name is
  unchanged from the word it replaces
  (`marketing-site`: *A control reduced to a glyph keeps its name and its target*).
- [ ] 2.3 Make the anchor `inline-flex` and give it `p-2 -m-2`, so the activation
  target reaches 32×32 while its layout contribution stays the mark's own 16px
  (`marketing-site`: *A control reduced to a glyph keeps its name and its target*).
- [ ] 2.4 Comment the load-bearing choices in this file's established idiom — why
...
- [ ] 3.1 Start the site yourself with `bun run site:dev` — never ask the user to
  run it — and confirm the rendered mark matches GitHub's reference glyph at 16px.
- [ ] 3.2 Compare the mark's optical weight against `Docs` in the same row, under
...
- [ ] 3.3 Measure the header row's intrinsic width at a 390px viewport before and
  ...
  estimated saving.
- [ ] 3.4 Confirm in the browser that the anchor's bounding box is ≥ 24px in each
  dimension and that its bounding box plus horizontal margins does not exceed the
  glyph's own width.

## 4. Tests

- [ ] 4.1 Extend the header test in `site/e2e/tests/routes.spec.ts` (currently
  ...
  (`marketing-site`: *A control reduced to a glyph keeps its name and its target*).
- [ ] 4.2 Add a guard to `site/e2e/tests/layout.spec.ts`, beside the existing
  ...
  (`marketing-site`: *A control reduced to a glyph keeps its name and its target*).
- [ ] 4.3 Assert the mark itself is hidden from assistive technology, so the link is
  announced once rather than twice
  (`marketing-site`: *A control reduced to a glyph keeps its name and its target*).
- [ ] 4.4 Confirm the existing narrow-viewport overflow tests at 320/360/375px still
...
- [ ] 5.1 `bun run --cwd site typecheck`
- [ ] 5.2 `bun run --cwd site check:spelling`
- [ ] 5.3 `bun run site:build`
- [ ] 5.4 `bun run site:test`
- [ ] 5.5 Manual smoke in `bun run site:dev`, run by the implementer, walking each
  ...
  375 or 390px.
- [ ] 5.6 Confirm no Rust crate, no `src/types.ts` entry and no `site/bun.lock` line
