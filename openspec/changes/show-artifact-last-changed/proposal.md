...
- **The application's relative-time vocabulary is unified into one tested module.** This was not in the original scope; implementation found two private copies already in the tree — `formatRelativeTime` in `WorkspaceTree.tsx` and `relativeTime` in `DashboardView.tsx` — which had *already* drifted apart ("12s ago" against "just now", weeks and months in one and not the other), plus a self-ticking `RelativeTime` component whose doc comment describes the very freeze problem this change had to solve. Adding a third copy would have put two spellings of one kind of value on screen at the same time, which is precisely the defect `tint-identity-branch-chip` shipped as a warning about. All three surfaces now read one definition, which gains the cadence function, width bound and tests that none of the copies had.
...
- `src/relativeTime.ts` (new) — the one relative-time vocabulary, plus its tick cadence and reserved-width bound, with tests. JSX is not exercised by `bun test` and a frontend-only diff short-circuits the mutation gate, so a pure function with tests is the only coverage this logic can get — the same reasoning that put `branchChipForWorktree` beside it.
- `src/hooks/useTickingNow.ts` (new) + `src/components/RelativeTime.tsx` (new) — one self-ticking implementation for all three surfaces.
- `src/components/WorkspaceTree.tsx`, `src/components/DashboardView.tsx` — their private formatters deleted, both now reading the shared module.
...
- **~~The Dashboard and the workspace tree are untouched.~~** *Superseded during implementation.* Both are touched, but only to **delete** their private relative-time formatters and read the shared one — no new signal is added to either surface. The tree already spelled out an elapsed time in words (`row-mtime`) and the Dashboard already did in its ships feed; this change did not notice until the header rendered a third spelling beside them. The one rendered difference is the tree's sub-minute case, which becomes `just now` instead of counting seconds — a tier that could only stay honest with a per-second timer the tree never had, so it displayed a frozen `12s ago` for up to a minute regardless. Both surfaces' existing requirements already document the vocabulary adopted here (`spec-browser`: "a relative modification time (e.g. `12m ago`)"; `dashboard`: "archived 2h ago"), so this moves both implementations *toward* their specs rather than away.

- **How the tree derives its time is untouched.** It still shows `ChangeInstance.modifiedAt` — the change directory's newest write — and still sorts and paints the active dot from it. Only the words change, not the value. That the tree's number and the header's differ for one change is correct and intended: they answer different questions.
