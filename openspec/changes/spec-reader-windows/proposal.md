...
- **A reader window: one document, no navigation.** A chromeless surface carrying the document and its identity header, and nothing else — no tree, no commit rail, no footer, no quota pills, no Settings or Archive entry points. Opened by Cmd/Ctrl-clicking a row in the workspace tree or in the file browser's folder tree. Dismissed with Cmd/Ctrl-W, with Escape, or with its own close control.
